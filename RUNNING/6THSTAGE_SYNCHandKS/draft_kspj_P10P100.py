    ...
    for a in range(1,M):
        ...
        for s in range(1,M):
            ...
            for m in range(1,M):
                ...
                #### Calculating 1st S (n time to be prognosticated at the end) ####
                if s == 1:
                    if m == (M-1):         
                        random = np.zeros(D)
            ...
            #### Constructing the 10th time evolution ######            
            if s == (M-1):            
                random = np.zeros(D)
                xx = mod.lorenz96(xx,random,dt) 

                S[:,a] = np.dot(h,xx)
                Y[:,a] = y[:,(a*M)+(n-1)]    #**********CHECK THIS!********# 

        #### Extracting initial Jac P1010 #####
        if a == 1:
            #P['00'] = P['99']
            newP['00'] = P['99']
