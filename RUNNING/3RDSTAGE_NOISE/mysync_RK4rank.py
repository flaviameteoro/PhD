...
### No noise for y (ok for seed=37 and seed=18)
y = np.dot(H,xtrue)
#y = np.dot(H,xtrue) + np.random.uniform(0,1.2680e-04,J+1)-6.34e-05
...
for t in range(run):
    if (t == tobs[mcn]): 
        ...
        ##############pinv_tol = 2.2204e-16  #CHANGING TO e-5 MAKES SE DECREASE A LITTLE(SEE PG 7, REY 2ND ARTICLE)
        ...
        svmaxvec[:,t] = svmax
        svmaxvec2[:,t] = G[1]
        difmax = abs(svmaxvec[:,t] - svmaxvec[:,t-1])
        difmax2 = abs(svmaxvec2[:,t] - svmaxvec2[:,t-1])
        ...
        plt.plot(t+1,svmin,'c<') 
        plt.hold(True)

        plt.plot(t+1,svmax,'r>') 
        plt.hold(True)

        #plt.plot(t+1,ratioobs,'yo') 
        #plt.hold(True)
      
        plt.plot(t+1,condnumber,'m.') 
        plt.hold(True)

        #plt.plot(t+1,obin,'m<') 
        #plt.hold(True)

        #plt.plot(t+1,difmax,'yo') 
        #plt.hold(True)

        #plt.plot(t+1,difmax2,'mo') 
        #plt.hold(True)
