...
sigma2 = 0.01
sig_mag = 1e-2           #### for searching the minimal SEs ########
...
fitrange = 100                   #### for the curve fitting calculation ####
#SEfit = np.zeros([1,fitrange])  #### for the curve fitting calculation ####
#xaxis = np.zeros([1,fitrange])  #### for the curve fitting calculation ####
SEfit = np.zeros((fitrange))     #### for the curve fitting calculation ####
xaxis = np.zeros((fitrange))     #### for the curve fitting calculation ####

SEmin = []                       #### for searching the minimal SEs ########
tsmin = []                       #### for searching the minimal SEs ########
coefmin = []                     #### for searching the minimal coefs ########
...
for n in range(1,run+1):
    ...
    ################ Storing SE for the curve fitting calculation ############# 
    if n <= fitrange:     
        #SEfit[:,n-1] = SE
        #xaxis[:,n-1] = n+1
        SEfit[n-1] = SE
    ...
    ########## Calculating a curve fitting (with a 1 degree polynomial) #######
    if n >= fitrange:  
        coefs = poly.polyfit(xaxis, SEfit, 1)
        ffit = poly.Polynomial(coefs)
        SEfit_fitt  = ffit(xaxis)
        #print 'Slope until time step', n+1, 'is', coefs[1]
    
    ######## Plotting fittings and slopes #################
        plt.figure(2)
        plt.plot(xaxis,SEfit,'o')
        plt.yscale('log')
        plt.hold(True)
        plt.plot(xaxis,SEfit_fitt,'m-')
  
        plt.figure(3)
        plt.plot(n+1,coefs[1],'g.')
        #plt.yscale('log')
        plt.hold(True)

    ####### Storing the minimal SEs ############
    if SE < sigma2 + sig_mag:
        SEmin.append(SE)
        tsmin.append(n+1)
        coefmin.append(coefs[1])

    #print 'SEmin', SEmin
    #print 'tsmin', tsmin
    #print 'coefmin', coefmin    

    plt.figure(4)
    #plt.plot(tsmin,SEmin,'r*')
    #plt.yscale('log')
    #plt.hold(True)
    plt.plot(tsmin,coefmin,'g*')
    plt.hold(True)    

SEtotmin = min(SEmin)
print 'The minimal SE is', SEtotmin

coeftotmin = min(coefmin)
print 'The minimal slope is', coeftotmin
