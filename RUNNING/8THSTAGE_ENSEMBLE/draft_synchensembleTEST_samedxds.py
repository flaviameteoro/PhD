...
dxds_generic = np.zeros([Nens,M*L]) # for testing the use of same dxds
Adxds_generic = np.zeros([D,M*L])   # for testing the use of same A.dxds
...
for n in range(1,run+1):
    ...
    ##############################################################################
    ###### Testing the use of the same dxds during the whole process #############
    ###### (needs to change also in dx2 calculation) #############################
    if n == 1:
        dxds_generic = dx1        
    #print 'dxds for', n, 'is', dxds_generic
    ##############################################################################
    ...
    dx2 = np.dot(A,dx1)
    ##dx2 = np.dot(A,dxds_generic)  # for testing the use of same dxds!!!!!!!!!!!
    #print 'dx2', dx2

    ##############################################################################
    ###### Testing the use of the same A.dxds during the whole process ###########
    ###### (needs to change also in dx3 calculation) #############################
    if n == 1:
        Adxds_generic = dx2        
    print 'Adxds for', n, 'is', Adxds_generic
    ##############################################################################
    ...
    for i in range(D):
        ...
        dx3 = np.dot(Adxds_generic[i,:],np.transpose(newdYS))   # for testing the use of same A.dxds!!!!!!!!!!!
