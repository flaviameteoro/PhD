...
for n in range(1,run+1):
    ...
    for i in range(D):
        ...
        for v in range(1,M+1):
            for j in range(L):
                #l = np.mod(j,L)*(L-1)
                l = np.mod(j,L)*(L-1) - (v-1)*nTau          # Considering the movement of the gridpoints/time step specifically for Lorenz96 model (for tau=10, 10 gridpoints)
                if l < 0:                                   # Considering the movement of the gridpoints/time step specifically for Lorenz96 model (for tau=10, 10 gridpoints)
                    l = l + D                               # Considering the movement of the gridpoints/time step specifically for Lorenz96 model (for tau=10, 10 gridpoints)
        ...
        #print 'dloc for var',i, ':', dloc
            ...
            for k in range(L):
                ...
                #dYS[:,v*(k-1)] = dloc[k]*dYS[:,v*k]            # SCHUR Product!! 
                dYS[:,k] = dloc[k]*dYS[:,k]                     # SCHUR Product!! 
            kl = k
            #print 'kl', kl
