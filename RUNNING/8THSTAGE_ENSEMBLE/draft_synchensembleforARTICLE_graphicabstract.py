...
#    plt.subplot(np.ceil(D/8.0),2,i+1)
    ...
    if np.mod(i,i_obs) == 0:   
        i_y = i/i_obs
        plt.plot(y[i_y,:],'y.',label='obs')   ## create y with no zeros to plot correctly ###
        plt.hold(True)

    ##################### Plotting the variables ###########################    
           
    plt.plot(x[i,:],'r',linewidth=2.0, label='X')
    plt.hold(True)
...
#for i in range(D):
    ...
    ##################### Plotting the variables ########################### 
    plt.plot(x[k,:],'r',linewidth=2.0, label='X')
    plt.hold(True)
...
################## Plotting the observations ###########################
plt.plot(y[2,:],'c.',label='obs')   ## create y with no zeros to plot correctly ###
plt.hold(True)

##################### Plotting the variables ###########################    
plt.plot(xtrue[8,:],'b-',linewidth=2.0,label='truth')
for tick in ax.xaxis.get_ticklabels():
...
for tick in ax.yaxis.get_ticklabels():
        ...
        #tick.set_weight('bold')
plt.hold(True)           
plt.plot(x[8,:],'r--',linewidth=2.0, label='X')
plt.hold(True)
#plt.plot(xtrue[8,:],'b-',linewidth=2.0,label='truth')
#plt.hold(True)
        
plt.ylabel('x[8]', fontsize=40)
plt.xlabel('time steps', fontsize=40)

#plt.savefig('var8eps', format='eps', dpi=1000)
#plt.savefig('var7test.png')
plt.show()
