...
for n in range(run):
    x[:,n+1] = mod.lorenz96(x[:,n],random6,dt) 
    z[:,n+1] = mod.lorenz96(z[:,n],random8,dt) 
    w[:,n+1] = mod.lorenz96(w[:,n],random10,dt) 
...
plt.figure(figsize=(12, 10)).suptitle('Lorenz free runs - Model noise influence for different standard deviations (Gaussian distributions)')
for i in range(D/3):
    ...
    plt.legend(['e-03', 'e-02', 'e-01', 'truth'], ncol=4, prop={'size':10}, loc='upper left')
