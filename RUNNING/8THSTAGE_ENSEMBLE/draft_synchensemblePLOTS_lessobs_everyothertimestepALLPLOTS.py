...
################################################################################
################ obs ONLY at each other time step (odd time steps)##############
...
E_even = np.zeros([D])         ### ensemble mean for even time steps###
...
for n in range(1,run+1):
    ...
    if np.mod(n,2) == 1:
        ...
        plt.figure(7).suptitle('Local Lyapunov Exponents (Nens='+str(Nens)+')')
        ...
        #*for i in range(D/3):
            ...
            plt.subplot(np.ceil(D/4.0),4,i+1)  
            if lya[i] >= 0:
        ...
        ####### Plotting total unstable and neutral Lyapunov exponents #######
        lyastore = 0 
        for i in range(D):
            if lya[i] >= 0:       
                lyastore = lyastore + 1
                #print 'lyastore', lyastore

        fig = plt.figure(5)
        ax = fig.add_subplot(1, 1, 1) # create an axes object in the figure
        plt.plot(n+1,lyastore,'mo') 

        plt.title('Total unstable and neutral Local Lyapunov exponents')
        plt.xlabel('Time units')        


    else:
        ##########################################################################################
        ########## 1st ATTEMPT: just evolve the even time step with the NL model #################
                                 ######### NO SYNCH... ##############
        ##########################################################################################
        #### Evolving variable with time #######       # it does not synchronise (too chaotic yet?)
        #random = np.zeros(D)
        ##x[:,n+1] = mod.rk4(x[:,n],dt) 



        #############################################################################################################
        ### 2nd ATTEMPT: generate an ensemble from the even time step and consider the next time step as its mean ###
                                        ######### NO SYNCH... ##############
        #############################################################################################################
        ################### Creating ensemble ##########################
        ###Ens_even = np.zeros([D,Nens])   
        ###for i in range(len(Ens_even[0,:])):
            ###force = 0.1*np.random.randn(D) + 0 
            #force = 0.5*np.random.randn(D)  
            #force = 2.5*np.random.randn(D)  
            #force = np.random.randn(D)  
            ###Ens_even[:,i] = x[:,n] + force        # ensemble created from x[:,n]


        ############ Calculate the ensemble mean to construct S ######## 
        ###for a in range(D):
            ###E_even[a] = np.mean(Ens_even[a,:])
        #print 'Mean for time', n, 'is', E_even
        #print 'x', n, 'is', x[:,n]
        ...
        ################################################################################################
        ####### 3rd ATTEMPT: run the NL model from the mean of the ensemble of the even time step ######
                ######## WORKING (not so well) FOR D=20 AND Nens=20... (max_rank=5) #############
        ################################################################################################
        #x[:,n] = E_even                           
        #x[:,n+1] = mod.rk4(x[:,n],dt) 
        ...
        ################################################################################################
        ########################### 4th ATTEMPT: repeat last dxds ######################################
                      ######## WORKING PERFECTLY FOR D=20 AND Nens=5 !!!!!!!!!!!! #############
        ################################################################################################
        x[:,n+1] = mod.rk4_end(x[:,n],DX,dt)
        ...
        #plt.figure(figsize=(12, 10)).suptitle('Lyapunov Exponents')
        #if [any(item) in lya >0 
        plt.figure(7).suptitle('Lyapunov Exponents (Nens='+str(Nens)+')')
        plt.axhline(y=0, xmin=0, xmax=run, linewidth=1, color = 'm')
        for i in range(D):   
        ###for i in range(D):                   # to plot all variables!!
            ###plt.subplot(D/4,4,i+1)           # to plot all variables!!
        ...
        plt.title('Total unstable and neutral Lyapunov exponents')
        plt.xlabel('Time units')        
