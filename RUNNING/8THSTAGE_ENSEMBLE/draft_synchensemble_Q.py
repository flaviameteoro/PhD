...
K_new = 1.e2*np.diag(np.ones([Nens]))      #Increased to 100 to prevent dx to be divided by a factor of 10!!!! (what made Localisation work for D=100 and D=1000) # For the 5th formulation 


Ks = 1.e0*np.diag(np.ones([L*M]))  

Q = 0.01                                     ########## ADDING model NOISE Q ##############
...
for n in range(1,run+1):
    ...
    for m in range(2,M+1):
        ...
        for i in range(1,int(nTau)+1):
            for j in range(len(Ens[0,:])):
                #random = np.zeros(D)
                ##random = Q*np.ones(D)                            ########## ADDING Q NOISE ############## 
                random = np.random.normal(0,Q,D)                   ########## ADDING NORMAL NOISE WITH VARIANCE=Q ############## 
                ####random = np.random.normal(0,Q)                   ########## ADDING NORMAL NOISE WITH VARIANCE=Q ############## 
    ...
    #x[:,n+1] = mod.rk4_end(x[:,n],DX,dt) 
    x[:,n+1] = mod.lorenz96_Q(x[:,n],random,DX,dt)     ########## ADDING Q NOISE ##############                                      
