...
D = 1000     # Note that max_pinv is now 2L (for all Ds)!
F=8.17
...
observed_vars = range(250)    ######MORE OBS#########
L = len(observed_vars) 
h = np.zeros([L,D])       
for i in range(L):          
...
#            plt.hold(True)
    ...
    plt.figure(7).suptitle('Positive and neutral Local Lyapunov Exponents')
    plt.axhline(y=0, xmin=0, xmax=run, linewidth=1, color = 'm')
    for i in range(D):   
    ###for i in range(D):                   # to plot all variables!!
        ###plt.subplot(D/4,4,i+1)           # to plot all variables!!
...
#plt.savefig('synchD100_biggerlabels.png')
plt.show()
...
#for k in range(D/100,D/50):   # for 1000 vars
#for k in range(D/1000,D/500):   # for 1000 vars
#for k in range(D/500,D/250):     # for 5000 vars
#for i in range(D/3):
#for i in range(D):
    #plt.subplot(np.ceil(D/8.0),2,i+1)
