...
for n in range(1,run+1):
    ...
    for i in range(D):
        ...
        for j in range(M*L):
            #l1 = np.mod(j,L)*(L-1)
            #if n == 1:                  ###what was suggested above to optimise didn't work #####
            l1 = np.mod(j,L)*(D/L)
            #print 'Initial l1', l1
...
plt.savefig('ensD1000optshortrun.png')
#plt.show()
...
plt.savefig('test1000opshortvar1.png')
...
plt.savefig('test1000shortopvar2.png')
#plt.show()
