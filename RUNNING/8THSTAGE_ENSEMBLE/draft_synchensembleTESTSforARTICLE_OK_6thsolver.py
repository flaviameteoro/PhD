...
for n in range(1,run+1):
    ...
    z = np.linalg.lstsq(dsdx, np.transpose((Y-S)))    # For the 6th working formulation - solver
    #print 'z', z
    #print z[2]
    #for i in z:
    z0 = z[0]  
    ...
    #dx1 = np.dot(K_new,dxds)         # For the 5th working formulation
    #dx2 = np.dot(K_new,z)             # For the 6th working formulation - solver
    dx2 = np.dot(K,A)             # For the 6th working formulation - solver
    #print 'dx2', dx2.shape

    #dx2 = np.dot(Ks,np.transpose((Y-S)))    # For all formulations apart from 5th
    #****dx2 = np.dot(dx1,np.transpose((Y-S)))    # For the 5th formulation

    #dx = np.dot(dx1,dx2)                     # For all formulations apart from 5th
    #dx = np.dot(A,dx2)                        # For the 5th formulations
    dx = np.dot(dx2,z0)                        # For the 6th formulations - solver
...
for k in range(D/100):   # for 1000 vars
#for i in range(D/3):
#for i in range(D):
    #plt.subplot(np.ceil(D/8.0),2,i+1)
    i2 = k - 10
